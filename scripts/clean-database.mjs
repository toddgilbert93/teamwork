...
// ── Default personas (same as seed.sql) ───────────────────────────────
...
async function main() {
  ...
  if (roomErr) {
    ...
    console.log('  ✅ Deleted all room_messages');
  }
  ...
  // 4. Re-insert the 6 default personas
  const { error: insertErr, data: inserted } = await supabase
    .from('personas')
    .insert(defaultPersonas)
    .select('id, name');
  if (insertErr) {
    console.error('  ❌ Failed to insert default personas:', insertErr.message);
  } else {
    console.log(`  ✅ Inserted ${inserted.length} default personas:`);
    for (const p of inserted) {
      console.log(`     • ${p.name} (${p.id})`);
    }
  }

  console.log('\n✨ Database cleaned! App should show a fresh state.\n');
}
