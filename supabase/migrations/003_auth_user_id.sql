-- ============================================================
-- Migration 003: Add user_id to all tables and enforce RLS
-- ============================================================
...
DROP POLICY IF EXISTS "Allow all on room_messages" ON room_messages;

-- 4. Create user-scoped RLS policies for personas
...
-- 5. Create user-scoped RLS policies for messages
...
-- 6. Create user-scoped RLS policies for room_messages
CREATE POLICY "Users can view own room_messages"
  ON room_messages FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own room_messages"
  ON room_messages FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can delete own room_messages"
  ON room_messages FOR DELETE
  USING (auth.uid() = user_id);

-- 7. Update RPC function (SECURITY INVOKER so RLS applies)
...
-- 8. Create function to seed default personas for new users
CREATE OR REPLACE FUNCTION seed_default_personas()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO personas (name, emoji, accent_color, system_prompt, tagline, sort_order, user_id)
  VALUES
  (
    'Sol',
    '☀️',
...
CREATE TRIGGER on_auth_user_created
  ...
  FOR EACH ROW
  EXECUTE FUNCTION seed_default_personas();
